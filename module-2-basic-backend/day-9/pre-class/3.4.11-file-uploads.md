...
In [3.1.3: POST Requests](../../../Module2/day9/3.1-express-js/3.1.3-handling-post-requests.md#receive-post-request-data-as-keys-and-values), we added an `express.urlencoded` middleware to populate `request.body`. For file uploads, we will add a similar configuration to Express using the [Multer](https://github.com/expressjs/multer) library to accept binary file data in requests.
