...
In React class components (see [7.2.3: Class Components](../../../Module4/day3/preclass/7.2-react-components/7.2.3-class-components.md)), there is a special class property `this.state` that is an object that stores the local state for that component. When properties in `this.state` change, the JSX that depends on those properties gets re-rendered.
